  ...
  useEffect(() => {
    const loadDashboard = async () => {
      try {
        ...
        setTasks(taskResponses.flatMap((response) => response.data.data.tasks));
      } catch (err) {
  ...
  return (
    <>
      <header className="mb-lg">
        ...
        <p className="font-body-lg text-body-lg text-on-surface-variant">Welcome back. Here's what's happening across your projects today.</p>
      </header>
                ...
                {projects.slice(0, 6).map((project) => (
                  <div key={project._id}>
                    <div className="mb-1 flex justify-between text-label-md">
                      ...
                      <span className="text-on-surface-variant">{project.progress || 0}%</span>
                    </div>
